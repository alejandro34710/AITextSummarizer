...
function useHistory() {
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = "historial-resumenes.json";
    a.click();
...
export default function App() {
  ...
  async function onSummarize() {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data?.error || "Error procesando el resumen");
  ...
  const filteredHistory = useMemo(() => {
    ...
    return history.filter(
      (h) =>
        h.input.toLowerCase().includes(s) || h.output.toLowerCase().includes(s)
    );
  }, [history, q]);
    ...
    <div className="min-h-screen relative bg-app text-slate-900 dark:text-slate-100">
      ...
      {(loading || progress > 0) && (
        <div className="relative z-10 h-1 w-full bg-black/10 dark:bg-white/10">
          <div
            className="h-1 bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-sky-400 transition-all duration-200"
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-5 pb-16">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="glass p-5 lg:p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="hidden md:flex gap-2">
                <span className="chip chip-muted">
                  {wordsCount(text)} palabras
                </span>
                <span className="chip chip-muted">
                  {text.length} caracteres
                </span>
                <span className="chip chip-muted">
                  ~{readingTimeMin(text)} min
                </span>
              </div>
            </div>
            ...
            {error && (
              <p className="mt-3 text-rose-500 dark:text-rose-300 font-medium">
                ⚠ {error}
              </p>
            )}

            <p className="mt-5 text-slate-600 dark:text-slate-400 text-sm">
              Tip: pega con <span className="kbd">Ctrl/Cmd + V</span> y ajusta
          ...
          <section ref={summaryRef} className="glass p-5 lg:p-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                <div className="hidden sm:block text-xs text-slate-600 dark:text-slate-400">
                  {meta && (
                    <>
                      Original: {meta.originalChars} · Resumen:{" "}
                      {meta.summaryChars} · Oraciones: {meta.sentences}
                    </>
                  )}
                </div>
                {/* tamaño de letra */}
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => setSummarySize((s) => Math.max(14, s - 2))}
                  >
                    A-
                  </button>
                  <button
                    className="chip chip-muted"
                    onClick={() => setSummarySize((s) => Math.min(22, s + 2))}
                  >
                    A+
                  </button>
                </div>
            ...
            {!summary && !loading && (
              <div className="text-slate-600 dark:text-slate-400 text-sm">
                ...
                <p>
                  Pega un texto y haz clic en{" "}
                  <span className="font-semibold">Generar resumen</span>.
                </p>
              </div>
            )}
            ...
            {summary && !loading && (
              <>
                ...
                <div className="mt-3 flex flex-wrap gap-3">
                  <button onClick={copySummary} className="btn btn-ghost">
                    Copiar
                  </button>
                  <button onClick={downloadTxt} className="btn btn-ghost">
                    Descargar .txt
                  </button>
                </div>
              </>
        ...
        <section className="mt-6 glass p-5 lg:p-6">
          ...
          {!history.length ? (
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Aún no hay resúmenes guardados. Cuando generes uno, se agregará
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-slate-600 dark:text-slate-400">
                        {fmtDateTime(h.createdAt)} · {h.originalWords} palabras
                        · ~{h.readTimeMin} min
                      </span>
                      <div className="flex gap-2">
                        <button
